...
/* 		 (int) fread(ebcbuf, 1, EBCBYTES, tapefp); */
            result = fread(ebcbuf, 1, EBCBYTES, tapefp);
            if (ferror(tapefp)) {
                SEGYREAD_TAPE_ERROR("ebcdic header");
                clearerr(tapefp);
            } else { /* Reset counter on successful tape IO */
...
#endif
//	if(dfs){
//        err = dfs_open(dfs, obj_tape, cmdbuf, S_IFREG | S_IWUSR | S_IRUSR, O_RDWR | O_CREAT, cid, 0, NULL, &obj_pipe);
//        d_iov_t iov;
//        d_sg_list_t sgl;
//
//        /** set memory location */
...
//        d_iov_set(&iov, (void *)ebcbuf, EBCBYTES);
//        sgl.sg_iovs = &iov;
//        err = dfs_write(dfs, obj_pipe, &sgl, 0, NULL);
//	}else{
...
//	}
    ...
    if(ENABLE_DFS){
        ...
        if (buff) {
            ...
            result = fread((char *) &tapebh, 1, BNYBYTES, tapefp);
            if (ferror(tapefp)) {
                SEGYREAD_TAPE_ERROR("binary header");
                clearerr(tapefp);
            } else { /* Reset counter on successful tape IO */

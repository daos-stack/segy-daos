	...
	*type = hdtype(*key);
	...
	if(*first==0) {
		...
		*first=1;
		...
		erewind(nheaderfp);
        	erewind(ntracefp);
              result = fread (&tr,HDRBYTES, 1, nheaderfp);
              result = fread (tr.data,FSIZE, *nt, ntracefp);
		gethval(&tr, indx, n_val);
	}
	...
	{ register int ix;
        	for (ix=0; ix<*ntr; ix++)
                result = fread (rec[ix],HDRBYTES, 1, headerfp);
        	efclose (headerfp);
		for(ix=0; ix<*ntr; ix++)
		     result = fread ((*rec[ix]).data,FSIZE, *nt, tracefp);
        	efclose (tracefp);
	}

...
void scanfval(register cwp_String type, Value *valp)
{
	...
	switch(*type) {
	case 's':
		result = scanf("%s", (char *) valp);
	break;
	case 'h':
	case 'u':
	case 'U':
		result = scanf("%hd", (short *) valp);
	break;
	case 'i':
	case 'p':
	case 'P':
		result = scanf("%d", (int *) valp);
	break;
	case 'l':
	case 'v':
		result = scanf("%ld", (long int *)  valp);
	break;
	case 'f':
		result = scanf("%f", (float *) valp);
	break;
	case 'd':
		result = scanf("%lf", (double *) valp);
	break;
	default:

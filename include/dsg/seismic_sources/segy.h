...
#ifndef DAOS_DAOS_SEISMIC_GRAPH_INCLUDE_DSG_SEISMIC_SOURCES_SEGY_H_
#define DAOS_DAOS_SEISMIC_GRAPH_INCLUDE_DSG_SEISMIC_SOURCES_SEGY_H_
...
typedef struct segy_parameters {
	bhed		*bh;
	char		*ebcbuf;
	char		*exthbuf;
	short		nextended;
...
/** Function responsible for initializing segy parameters.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
...
/** Function responsible for releasing segy previously
...
/** Function responsible parsing and processing segy binary and text headers
 *  from a mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
 * 						used to release any allocated
 ...
 */
int
parse_segy_bh_th(void *segy_parameters);

/** Function responsible for initializing segy parsing function pointers.
 *
 * /param[in]		parsing_fucntions	void pointer to be allocated
 * 						and initialized with all
 * 						segy parsing function
 * 						pointers.
 *
 * /return	0 on success
 * 		error code otherwise.
...
/** Function responsible getting previously parsed text header from a
 *  mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
...
/** Function responsible getting previously parsed binary header from a
 *  mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
...
/** Function responsible getting number of extended file headers in a
 *  mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
...
/** Function responsible for parsing, processing if needed and returning
 *  extended text header from a mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct and
 * 						used to release any allocated
...
/** Function responsible for parsing, processing if needed a trace
 *  from a mounted segy file.
 *
 * /param[in]		parameters		void pointer to be casted to
 * 						segy_parameters struct.
 *
...
/** Function responsible for converting segy struct to trace_t struct
 *
 * /param[in]		trace			pointer to trace struct to be
 * 						returned.
 * /param[in]		segy_trace		pointer to segy struct to be
 * 						converted to trace_t
 *
 * /return	0 on success
 * 		error code otherwise.
 */
...
#endif /* DAOS_DAOS_SEISMIC_GRAPH_INCLUDE_DSG_SEISMIC_SOURCES_SEGY_H_ */

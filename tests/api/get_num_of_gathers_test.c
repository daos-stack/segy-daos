...
void
gather_obj_get_num_of_gathers_test(void **state)
{
	root_obj_t 	*root_obj;
	int		curr_idx;
	int		rc;
	...
	int fetched_num_of_gathers = daos_seis_get_num_of_gathers(root_obj, "fldr");
	rc = daos_seis_close_graph(root_obj);
	assert_int_equal(rc, 0);
	assert_int_equal(fetched_num_of_traces, 12010);
	assert_int_equal(fetched_num_of_gathers, 10);
	return;
}

void
gather_obj_get_num_of_gathers_non_existing_key_test(void **state)
{
	root_obj_t 	*root_obj;
	int		curr_idx;
	int		rc;
	...
	int fetched_num_of_gathers = daos_seis_get_num_of_gathers(root_obj, "cdp");
	rc = daos_seis_close_graph(root_obj);
	assert_int_equal(rc, 0);
	assert_int_equal(fetched_num_of_traces, 12010);
	assert_int_equal(fetched_num_of_gathers, 1273);
	return;
}
...
int main(int argc, char* argv[])
{
	...
	if(container_string[13] == 'f') {
		container_string[13] = 'a';
	} else if(container_string[13] == '9') {
		container_string[13] = '1';
	} else {
		container_string[13] = container_string[13] + 1;
	}
  ...
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(gather_obj_get_num_of_gathers_test),
    cmocka_unit_test(gather_obj_get_num_of_gathers_non_existing_key_test)
  };

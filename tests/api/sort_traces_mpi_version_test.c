/*
 * sort_traces_mpi_version_test.c
 *
...
//	printf("FInished sorting traces \n");
	if(process_rank == 0) {
...
sort_complex_headers_with_window_test(void** state){
	...
	if(process_rank == 0){
		assert_non_null(e);
		assert_int_equal(e->ensembles->size, 2);
		node_t* ensemble_node = e->ensembles->head;
		node_t* trace_node;
		ensemble_t* temp_ensemble;
		trace_t* temp_trace;
		...
		while(ensemble_node != NULL){
			...
			temp_ensemble =
				doubly_linked_list_get_object(ensemble_node,
								  offsetof(ensemble_t,n));
			if(i == 0){
				assert_int_equal(temp_ensemble->traces->size,3603);
				i++;
			}
			else{
				assert_int_equal(temp_ensemble->traces->size,5995);
			}
			trace_node = temp_ensemble->traces->head;

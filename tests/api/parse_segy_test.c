...
static void parse_segy_additional_test(void **state)
{
	int	rc;
	int i;
	root_obj_t	*root_obj;
...
int main(int argc, char* argv[])
{
	...
	if(container_string[14] == 'f') {
		container_string[14] = 'a';
	} else if(container_string[14] == '9') {
		container_string[14] = '1';
	} else {
		container_string[14] = container_string[14] + 1;
	}
  ...
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(parse_segy_additional_test)
  };

...
//	dfs_obj_t * parent = get_parent_of_file("/test", 1, file_name);
...
void
update_graph_num_of_traces_test(void **state)
{
	root_obj_t *root_obj;
	int	curr_idx;
	int		rc;
	...
	//root_obj_open(&root_obj->oid_oh,root_obj->daos_mode,NULL,-1,&curr_idx);
	rc = root_obj_update_num_of_traces(root_obj, NULL,&(root_obj->num_of_traces),
					   -1, &curr_idx);
	//root_obj_close(root_obj,NULL,-1,&curr_idx,0);
	rc = daos_seis_close_graph(root_obj);
	root_obj  = daos_seis_open_graph("/test", O_RDWR);
...
int main(int argc, char* argv[])
{
	...
	if(container_string[11] == 'f') {
		container_string[11] = 'a';
	} else if(container_string[11] == '9') {
		container_string[11] = '1';
	} else {
		container_string[11] = container_string[11] + 1;
	}

  const struct CMUnitTest tests[] = {
    cmocka_unit_test(create_graph_test),
    cmocka_unit_test(update_graph_num_of_traces_test)
  };

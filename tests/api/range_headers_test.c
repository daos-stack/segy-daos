...
static void parse_segy_test(void **state)
{
	...
	daos_seis_close_graph(root_obj);
	free(tr);
	free(keys[0]);
	free(keys);
...
static void
range_headers_test(void** state){
	root_obj_t	*root_obj;
	int		num_of_keys = 2;
	int		rc;
	int		i;
	int		curr_idx;
	char		**range_keys = malloc(num_of_keys * sizeof(char *));

	for(i = 0; i < num_of_keys; i++){
		range_keys[i] = malloc(MAX_KEY_LENGTH * sizeof(char));
	}

	strcpy(range_keys[0], "tracl");
	strcpy(range_keys[1], "fldr");
	headers_ranges_t* rng;
	root_obj  = daos_seis_open_graph("/test", O_RDWR);
	rng = daos_seis_range(root_obj, num_of_keys, range_keys, 1);
	daos_seis_close_graph(root_obj);
	assert_non_null(rng);
	assert_int_equal(rng->trmin->fldr, 601);
	assert_int_equal(rng->trlast->fldr, 610);
	assert_int_equal(rng->trfirst->tracl, 1);
...
int main(int argc, char* argv[])
{
	...
	if(container_string[22] == 'f') {
		container_string[22] = 'a';
	} else if(container_string[22] == '9') {
		container_string[22] = '1';
	} else {
		container_string[22] = container_string[22] + 1;
	}
  ...
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(range_headers_test)
  };
